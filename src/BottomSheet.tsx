...
function BottomSheet({
	...
	useDerivedValue(() => {
		...
		offset.value = withSpring(0, { stiffness: 900, damping: 90, mass: 4 }, (finished) => {
			if (finished) {
	...
	const pan = Gesture.Pan()
		.onChange((event) => {
			...
			const nextBackdrop = interpolate(nextOffset, [0, height.value], [1, 0], Extrapolation.CLAMP);
	...
	const backdropOpacityStyle = useAnimatedStyle(() => ({ opacity: backdropOpacity.value }));
	...
	const sheetExiting = () => {
		...
		const animations = {
			transform: [{ translateY: withTiming(exitTo, { duration: needsMove ? 250 : 0 }) }],
			opacity: withTiming(1)
		};

		// Iniciar desde donde realmente está el sheet, no desde 0
		const initialValues = { transform: [{ translateY: start }], opacity: containerOpacity.value };
